...
function Simulation({sim,agentRun,days,setDays,seed,setSeed,running,setRunning,metrics,pilot,info,reset,buyerApi,error,busy,nextDay,progress,progressConnected}){
  const [recordDay,setRecordDay]=useState('latest');
  ...
  return <section className="simulation">
    ...
    <div className="keybox"><b>市場 agents</b><span>{buyerApi.enabled?`GPT 已連線 · ${buyerApi.model}`:'GPT 尚未連線，請確認伺服器設定'}</span><small>買家與競品各自使用 GPT 決策；店長沿用後台 GPT 設定。第一天尚無昨日交易，以零銷售作為起始資料。</small></div>
