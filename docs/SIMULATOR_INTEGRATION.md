...
1. 第一次推進時，GPT 依成本、品質與競品價格替六項商品建立初始售價並上架。
2. 模擬器產生當日需求事件、競品狀態、AI 買家交易、營收、毛利與評論。
3. 下一次推進時，MarketPilot 讀取每個 SKU 前一日成交與最近七日歷史。
4. GPT 分別決定售價、廣告預算、折扣、促銷強度與建議補貨量。
5. 毛利率、每日價格變動、廣告、折扣及促銷上限由程式再次限制。
6. 售價與行銷策略自動送回模擬器；廣告費會計入當日利潤，補貨量先通知賣家，確認後排入下一日。
7. 模擬器使用新策略推進一天，結果回到後台形成下一輪資料。

## 後台 API

- `GET /api/simulator/dashboard`：市場、商品、事件與最新 AI 決策。
- `POST /api/simulator/run-day`：完成一次 AI 決策與市場日結。
