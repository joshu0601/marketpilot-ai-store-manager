...
1. 後台商品完成 GPT 初始定價後會立即同步至模擬器商品目錄與消費者商城；既有商品會自動補同步。
2. 第一次推進時，GPT 依成本、品質與競品價格替尚未上架的內建商品建立初始售價。
3. 模擬器產生當日需求事件、競品狀態、AI 買家交易、營收、毛利與評論。
4. 下一次推進時，MarketPilot 讀取每個 SKU 前一日成交與最近七日歷史。
5. GPT 分別決定售價、廣告預算、折扣、促銷強度與建議補貨量。
6. 毛利率、每日價格變動、廣告、折扣及促銷上限由程式再次限制。
7. 售價與行銷策略自動送回模擬器；廣告費會計入當日利潤，補貨量先通知賣家，確認後排入下一日。
